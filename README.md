...
by now you should have `image_tsne_projections.json` and `images_..jpg` those are your atlases next is to create new folder inside `/src/assets` and copy it inside and to modify `config.js` inside `/src/` folder that will fit your model, modify `main.js` and `index.html` to have your config selectable inside web page <br /> <br />
